...
@app.get("/bandoru/<bandoru_id>/failed_webhooks")
@tracer.capture_method
def get_bandoru_failed_webhooks(bandoru_id: str):
    username = get_username_from_headers(app.current_event.headers)
    failed_webhooks = bandoru_service.get_failed_webhooks(bandoru_id, username)
    if failed_webhooks is None or len(failed_webhooks) == 0:
        return None, 204

    return failed_webhooks, 200
